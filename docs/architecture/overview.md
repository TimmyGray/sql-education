...
graph TD
    ...
    API -->|SQL execution| SandboxDB[("PostgreSQL :5433\n(sandbox — isolated)")]
...
| `SandboxModule` | Runs untrusted SQL on isolated Postgres with restricted role | `apps/api/src/sandbox/` |
...
5. `GradingService.grade` runs the 3-stage pipeline:
   ...
   - **Execute:** `PgSandboxRunner.runGraded` on isolated Postgres — 2-second timeout, `sandbox_runner` role (read-only)
...
| SQL execution | Isolated Postgres with restricted `sandbox_runner` role — no INSERT/UPDATE/DELETE |
...
- [ADR-0002](decisions/0002-isolated-sql-sandbox.md) — Isolated PostgreSQL instance for untrusted SQL execution
